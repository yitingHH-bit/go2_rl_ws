...
- 固定 5 Hz（可调）保持你原有的到达目标/碰撞判定与速度映射

- MODEL_PATH=/path/to/actor.onnx  # 若留空则默认 ./actor.onnx找不到再尝试 ./actor.ts
- CTRL_HZ=5.0
- SCAN_TOPIC=/processed_scan
- ODOM_TOPIC=/odom
- GATE_TOPIC=/forward_or_stop
- REQUIRE_GOAL=1|0, AUTO_ARM=0|1
- REAL_LIDAR_DISTANCE_CAP=6.0, REAL_THRESHOLD_COLLISION=0.19, REAL_THRESHOLD_GOAL=0.30
...
REAL_THRESHOLD_GOAL       = float(os.getenv("REAL_THRESHOLD_GOAL",      "0.20"))

REAL_SPEED_LINEAR_MAX   = float(os.getenv("REAL_SPEED_LINEAR_MAX",  "0.40"))
REAL_SPEED_ANGULAR_MAX  = float(os.getenv("REAL_SPEED_ANGULAR_MAX", "1.00"))
...
class LocalInferNode(Node):
    def __init__(self, model_path: str):
        ...
        # 定时器
        self.timer = self.create_timer(1.0/max(0.1, CTRL_HZ), self.control_loop)
        ...
        self.get_logger().info(f"[CFG] scan_topic={self.scan_topic} odom_topic={odom_topic} require_goal={self.require_goal} auto_arm={self.auto_arm}")
    ...
    def control_loop(self):
        ...
        if self.local_step > 15 and self._check_stop_conditions():
            return
        if not self.motion_enabled:
            self.cmd_self._publish_zero(); return
